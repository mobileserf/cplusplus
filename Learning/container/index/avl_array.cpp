...
class avl_array
{
  ...
  alignas(128)Key         key_[Size];                 // node key
  alignas(128)T           val_[Size];                 // node value
  alignas(128)std::int8_t balance_[Size];             // subtree balance
...
public:
   ...
   */
  bool insert(const key_type& key, const value_type& val)  __attribute__((noinline))
  {
    if (root_ == INVALID_IDX) {
      key_[size_]     = key;
      val_[size_]     = val;
   ...
   */
  inline bool find(const key_type& key, value_type& val) const  __attribute__((noinline))
  {
   ...
   */
  inline iterator find(const key_type& key)  __attribute__((noinline))
  {
    for (size_type i = root_; i != INVALID_IDX;) {
      if (key < key_[i]) {
        i = child_[i].left;
   ...
   */
  inline bool erase(const key_type& key)  __attribute__((noinline))
  {
   ...
   */
  bool check() const  __attribute__((noinline))
  {
    // check root
...
private:
  ...
  void insert_balance(size_type node, std::int8_t balance)  __attribute__((noinline))
  {
  ...
  void delete_balance(size_type node, std::int8_t balance)  __attribute__((noinline))
  {
    while (node != INVALID_IDX) {
      balance = (balance_[node] += balance);
  ...
  size_type rotate_left(size_type node)  __attribute__((noinline))
  {
    const size_type right      = child_[node].right;
  ...
  size_type rotate_right(size_type node)  __attribute__((noinline))
  {
    const size_type left       = child_[node].left;
  ...
  size_type rotate_left_right(size_type node)  __attribute__((noinline))
  {
    const size_type left             = child_[node].left;
    const size_type left_right       = child_[left].right;
  ...
  size_type rotate_right_left(size_type node)  __attribute__((noinline))
  {
    const size_type right            = child_[node].right;
    const size_type right_left       = child_[right].left;
...
#ifdef HASH
    std::unordered_map< std::string, avl_array<int, int, std::uint16_t, MAX_ENTRY_IN_AVL, true> > hash_map;
#else
    std::vector<avl_array<int, int, std::uint16_t, MAX_ENTRY_IN_AVL, true>* > hash_map(maxkey);
#endif
    for(int i = 0 ; i < maxkey; ++i) {
       std::string str = printRandomString(10);
       keys.push_back(str);
#ifdef HASH
       hash_map.emplace(str, avl_array<int, int, std::uint16_t, MAX_ENTRY_IN_AVL, true>());
#else
       hash_map[i] = new avl_array<int, int, std::uint16_t, MAX_ENTRY_IN_AVL, true>();
...
#ifdef N_PREFECTH
    for( ; i < maxkey*MAX_OPERATION; ++i) {
       int keypos = rand() % maxkey;
#ifdef HASH
       auto& value = hash_map[keys[keypos]];
#else
       auto& value = hash_map[keypos];
#endif
