...
dnl
dnl MSERF_CONFIG_NICE(filename)
dnl
dnl Saves a snapshot of the configure command-line for later reuse
dnl
AC_DEFUN([MSERF_CONFIG_NICE], [
  rm -f $1
  cat >$1<<EOF
#! /bin/sh
#
...
dnl
dnl MSERF_SETIFNULL(variable, value)
dnl
dnl  Set variable iff it's currently null
dnl
AC_DEFUN([MSERF_SETIFNULL], [
  if test -z "$$1"; then
...
dnl
dnl MSERF_SETVAR(variable, value)
dnl
dnl  Set variable no matter what
dnl
AC_DEFUN([MSERF_SETVAR], [
  test "x$verbose" = "xyes" && echo "  forcing $1 to \"$2\""
  $1="$2"
])dnl

dnl
dnl MSERF_ADDTO(variable, value)
dnl
dnl  Add value to variable
dnl
AC_DEFUN([MSERF_ADDTO], [
  if test "x$$1" = "x"; then
    test "x$verbose" = "xyes" && echo "  setting $1 to \"$2\""
    $1="$2"
  else
...
dnl
dnl MSERF_ADDTO_RPATH(path)
dnl
dnl   Adds path to variable with the '-rpath' directive.
dnl
AC_DEFUN([MSERF_ADDTO_RPATH], [
  AC_MSG_NOTICE([adding $1 to RPATH])
  MSERF_ADDTO(AM_LDFLAGS, [-R$1])
])dnl

dnl
dnl MSERF_REMOVEFROM(variable, value)
dnl
dnl Remove a value from a variable
dnl
AC_DEFUN([MSERF_REMOVEFROM], [
...
dnl
dnl MSERF_TRY_COMPILE_NO_WARNING(INCLUDES, FUNCTION-BODY,
...
dnl
AC_DEFUN([MSERF_TRY_COMPILE_NO_WARNING],
...
dnl
dnl MSERF_LINK_WITH_FLAGS_IFELSE(LIBS, FUNCTION-BODY,
dnl                           [ACTIONS-IF-LINKS], [ACTIONS-IF-LINK-FAILS])
dnl
dnl Tries a link test with the provided flags.
dnl

AC_DEFUN([MSERF_LINK_WITH_FLAGS_IFELSE],
[ats_save_LIBS=$LIBS
 LIBS="$LIBS $1"
...
dnl bar='${foo}/2'
dnl baz='${bar}/3'
dnl MSERF_EXPAND_VAR(fraz, $baz)
dnl   $fraz is now "1/2/3"
dnl
AC_DEFUN([MSERF_EXPAND_VAR], [
ats_last=
ats_cur="$2"
...
dnl MSERF_PATH_RELATIVE(final_path, $orig_path, $prefix)
dnl    $final_path now contains "bar"
AC_DEFUN([MSERF_PATH_RELATIVE], [
...
dnl MSERF_SUBST(VARIABLE)
dnl Makes VARIABLE available in generated files
dnl (do not use @variable@ in Makefiles, but $(variable))
AC_DEFUN([MSERF_SUBST], [
  MSERF_VAR_SUBST="$MSERF_VAR_SUBST $1"
  AC_SUBST($1)
])

dnl
dnl MSERF_SUBST_LAYOUT_PATH
dnl Export (via MSERF_SUBST) the various path-related variables that
dnl trafficserver will use while generating scripts and
dnl the default config file.
AC_DEFUN([MSERF_SUBST_LAYOUT_PATH], [
  MSERF_EXPAND_VAR(exp_$1, [$]$1)
  MSERF_PATH_RELATIVE(rel_$1, [$]exp_$1, ${prefix})
  MSERF_SUBST(exp_$1)
  MSERF_SUBST(rel_$1)
  MSERF_SUBST($1)
])

dnl MSERF_HELP_STRING(LHS, RHS)
...
AC_DEFUN([MSERF_HELP_STRING], [ifelse(regexp(AC_ACVERSION, 2\.1), -1, AC_HELP_STRING([$1],[$2]),[  ][$1] substr([                       ],len($1))[$2])])

dnl
dnl MSERF_LAYOUT(configlayout, layoutname [, extravars])
dnl
AC_DEFUN([MSERF_LAYOUT], [
...
dnl
dnl MSERF_ENABLE_LAYOUT(default layout name [, extra vars])
dnl
AC_DEFUN([MSERF_ENABLE_LAYOUT], [
AC_ARG_ENABLE(layout,
  [MSERF_HELP_STRING([--enable-layout=LAYOUT],[Enable LAYOUT specified inside config.layout file (defaults to TrafficServer)])],[
  LAYOUT=$enableval
])
...
MSERF_LAYOUT($srcdir/config.layout, $LAYOUT, $2)
...
dnl MSERF_CHECK_SOCKOPT(socket-option, [action-if-found], [action-if-not-found]
AC_DEFUN([MSERF_CHECK_SOCKOPT], [
