...
if test "$enable_jemalloc" != "no"; then
  ...
  if test "$jemalloc_base_dir" != "/usr"; then
    MSERF_ADDTO(CPPFLAGS, [-I${jemalloc_include}])
    MSERF_ADDTO(LDFLAGS, [-L${jemalloc_ldflags}])
    MSERF_ADDTO(LDFLAGS, [-Wl,--add-needed -L${jemalloc_base_dir}/lib -Wl,-rpath,${jemalloc_base_dir}/lib -Wl,--no-as-needed])
    MSERF_ADDTO_RPATH(${jemalloc_ldflags})
  fi
