...
dnl
dnl MSERF_CHECK_ZLIB: look for zlib libraries and headers
dnl
AC_DEFUN([MSERF_CHECK_ZLIB], [
enable_zlib=no
...
if test "$enable_zlib" != "no"; then
  ...
  if test "$zlib_base_dir" != "/usr"; then
    MSERF_ADDTO(CPPFLAGS, [-I${zlib_include}])
    MSERF_ADDTO(LDFLAGS, [-L${zlib_ldflags}])
    MSERF_ADDTO_RPATH(${zlib_ldflags})
  fi
