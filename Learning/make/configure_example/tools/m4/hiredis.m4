...
dnl
dnl MSERF_CHECK_HIREDIS: look for hiredis libraries and headers
dnl

AC_DEFUN([MSERF_CHECK_HIREDIS], [
...
if test "$has_hiredis" != "0"; then
  ...
  if test "$hiredis_base_dir" != "/usr"; then
    MSERF_ADDTO(CPPFLAGS, [-I${hiredis_include}])
    MSERF_ADDTO(LDFLAGS, [-L${hiredis_ldflags}])
    MSERF_ADDTO_RPATH(${hiredis_ldflags})
  fi
