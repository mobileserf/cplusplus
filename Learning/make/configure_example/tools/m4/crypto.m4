...
dnl
dnl MSERF_CHECK_CRYPTO: look for crypto libraries and headers
dnl
AC_DEFUN([MSERF_CHECK_CRYPTO], [
  ...
  if test "x${enable_crypto}" = "xyes"; then
    MSERF_ADDTO(LDFLAGS, [$OPENSSL_LDFLAGS])
    MSERF_ADDTO(CPPFLAGS, [$OPENSSL_INCLUDES])
    MSERF_ADDTO(CPPFLAGS, [-DOPENSSL_NO_SSL_INTERN])
  fi
...
dnl
AC_DEFUN([MSERF_CHECK_CRYPTO_VERSION], [
...
dnl
AC_DEFUN([MSERF_CHECK_CRYPTO_ASYNC], [
  ...
  MSERF_ARG_ENABLE_VAR([use], [tls-async])
...
dnl
AC_DEFUN([MSERF_CHECK_CRYPTO_HELLO_CB], [
...
#include <openssl/tls1.h>
  ...
  MSERF_ARG_ENABLE_VAR([use], [hello-cb])
...
dnl Since OpenSSL 1.1.0
dnl
AC_DEFUN([MSERF_CHECK_CRYPTO_SET_RBIO], [
...
#include <openssl/tls1.h>
  ...
  MSERF_ARG_ENABLE_VAR([use], [set-rbio])
...
dnl Since OpenSSL 1.1.0
dnl
AC_DEFUN([MSERF_CHECK_CRYPTO_DH_GET_2048_256], [
  _dh_saved_LIBS=$LIBS
...
#include<openssl/dh.h>
  ...
  MSERF_ARG_ENABLE_VAR([use], [dh_get_2048_256])
  AC_SUBST(use_dh_get_2048_256)
])

AC_DEFUN([MSERF_CHECK_CRYPTO_HKDF], [
  enable_hkdf=no
...
# error no EVP_PKEY_CTX_hkdf_mode support
  ...
  MSERF_ARG_ENABLE_VAR([use], [hkdf])
  AC_SUBST(use_hkdf)
])

AC_DEFUN([MSERF_CHECK_CRYPTO_TLS13], [
  enable_tls13=yes
...
# error no TLS1_3 support
  ...
  MSERF_ARG_ENABLE_VAR([use], [tls13])
  AC_SUBST(use_tls13)
])
...
dnl Since OpenSSL 1.1.0
dnl
AC_DEFUN([MSERF_CHECK_CRYPTO_OCSP], [
  ...
  MSERF_ARG_ENABLE_VAR([use], [tls-ocsp])
...
dnl Since OpenSSL 1.1.1
dnl
AC_DEFUN([MSERF_CHECK_CRYPTO_SET_CIPHERSUITES], [
  ...
  MSERF_ARG_ENABLE_VAR([use], [tls-set-ciphersuites])
...
dnl Since OpenSSL 1.1.1
dnl
AC_DEFUN([MSERF_CHECK_EARLY_DATA], [
  _set_ciphersuites_saved_LIBS=$LIBS
