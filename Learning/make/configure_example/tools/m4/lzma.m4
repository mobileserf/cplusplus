...
dnl
dnl MSERF_CHECK_LZMA: look for lzma libraries and headers
dnl
AC_DEFUN([MSERF_CHECK_LZMA], [
enable_lzma=no
...
if test "$enable_lzma" != "no"; then
  ...
  if test "$lzma_base_dir" != "/usr"; then
    MSERF_ADDTO(CPPFLAGS, [-I${lzma_include}])
    MSERF_ADDTO(LDFLAGS, [-L${lzma_ldflags}])
    MSERF_ADDTO_RPATH(${lzma_ldflags})
  fi
